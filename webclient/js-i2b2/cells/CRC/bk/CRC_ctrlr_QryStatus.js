			...
			if (results.error) {
				...
				for (var i=0; i<l; i++) {			
					...
					//	$('infoQueryStatusText').innerHTML += "<div style=\"margin-left: 20px; clear: both; height: 16px; line-height: 16px;\">";
						self.dispDIV.innerHTML += "<div style=\"clear: both; margin-left: 20px; float: left; height: 16px; line-height: 16px;\">" + params[i2].getAttribute("column") + ": <font color=\"#0000dd\">" + params[i2].firstChild.nodeValue;  + "</font></div>";
		...
		for (var i in self.QRS) {
			...
			if (rec.QRS_time) {
				...
				switch(rec.QRS_Status) {
					case "ERROR":
						self.dispDIV.innerHTML += '<div style="clear:both; height:16px; font-weight:bold; line-height:16px; "><div style="float:left; height:16px; line-height:16px; ">'+rec.title+'</div>';
						self.dispDIV.innerHTML += '<div style="float:right; height:16px; line-height:16px; "><font color="#dd0000">ERROR</font>['+rec.QRS_time+' secs]</div>';
						break;
					case "COMPLETED":
					case "FINISHED":
					...
					case "PROCESSING":
						self.dispDIV.innerHTML += '<div style="clear:both; height:16px; font-weight:bold; line-height:16px; "><div style="float:left; height:16px; line-height:16px; ">'+rec.title+'</div>';	
						self.dispDIV.innerHTML += '<div style="float:right; height:16px; line-height:16px; "><font color="#00dd00">PROCESSING</font>['+rec.QRS_time+' secs]</div>';
		...
		if (undefined != self.QI.message)
		{
			...
			for(i3 = 1; i3 < mySplitResult.length; i3++){
				...
				for (var i2 = 0; i2 < xml_v.childNodes.length; i2++) {
					try { 
					self.dispDIV.innerHTML += '<div style="margin-left:20px; clear:both; line-height:16px; ">' + i2b2.h.XPath(xml_v, 'descendant::name/text()/..')[i2].firstChild.nodeValue + '<font color="#0000dd">: ' + i2b2.h.XPath(xml_v, 'descendant::total_time_second/text()/..')[i2].firstChild.nodeValue + ' secs</font></div>';
